...
func get_data_page(page_num : int, http_requester: HTTPRequest, 
					sort: String, searched_player: String) -> void:
	if(searched_player == ""):
		http_requester.request("http://atesting7723.eu.pythonanywhere.com/leaderboard?page=" 
								+ String(page_num) + "&sort=" + sort)
	else:
		http_requester.request("http://atesting7723.eu.pythonanywhere.com/leaderboard/" 
								+ searched_player + "?page=" + String(page_num) + "&sort=" + sort)
